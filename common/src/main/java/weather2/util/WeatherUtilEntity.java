...
public class WeatherUtilEntity {
    ...
    public static boolean isPosOutside(World parWorld, Vec3 parPos, boolean cheapCheck) {
        ...
        if (WeatherUtilBlock.getPrecipitationHeightSafe(parWorld, new BlockPos(MathHelper.floor(parPos.xCoord), 0, MathHelper.floor(parPos.zCoord))).getY() < parPos.yCoord + 1)
            return true;
    ...
    public static boolean checkVecOutside(World parWorld, Vec3 parPos, Vec3 parCheckPos) {
        ...
        if (dirNorth) {
            if (WeatherUtilBlock.getPrecipitationHeightSafe(parWorld, new BlockPos(MathHelper.floor(parCheckPos.xCoord), 0, MathHelper.floor(parCheckPos.zCoord))).getY() < parCheckPos.yCoord)
                return true;
        }
        return false;
    }
