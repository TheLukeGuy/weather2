...
public class WorldEvent {
    ...
    public boolean checkForActiveInvadersCached() {
        if (lastCheckedInvasionCount == 0 || DimensionManager.getWorld(dimensionID).getWorldTime() % 100 == 0) {
            ...
            if (lastCheckedInvasionCount != invasionEntities.size() && invasionEntities.size() == 0)
    ...
    public void writeNBT(NBTTagCompound data) {
        ...
        int dist = -1;
        if (coordDestination != null && invasionEntities.size() > 0)
            dist = (int) ((EntityLivingBase) invasionEntities.get(0)).getDistance(coordDestination.posX, coordDestination.posY, coordDestination.posZ);
