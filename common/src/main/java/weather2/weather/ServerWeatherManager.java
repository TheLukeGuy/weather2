...
public class ServerWeatherManager extends WeatherManager {
    ...
    public void tick() {
        ...
        if (world != null) {
            ...
            if (world.getTotalWorldTime() % rate == 0) {
                ...
                if (!ConfigMisc.Aesthetic_Only_Mode && WeatherUtilConfig.listDimensionsClouds.contains(world.provider.getDimension())) {
                    for (int i = 0; i < world.playerEntities.size(); i++) {
                        ...
                        //layer 0
                        if (getCloudStormsByLayer(0).size() < ConfigStorm.Storm_MaxPerPlayerPerLayer * world.playerEntities.size()) {
                            if (rand.nextInt(5) == 0) {
                        ...
                        //layer 1
                        if (getCloudStormsByLayer(1).size() < ConfigStorm.Storm_MaxPerPlayerPerLayer * world.playerEntities.size()) {
            ...
            if (!ConfigMisc.Aesthetic_Only_Mode && !ConfigSand.Storm_NoSandstorms && WeatherUtilConfig.listDimensionsStorms.contains(world.provider.getDimension()) && world.getTotalWorldTime() % 200 == 0 && windManager.isHighWindEventActive()) {
                Random rand = new Random();
    ...
    public boolean trySpawnSandstormNearPos(World world, Vec3 posIn) {
        ...
        for (int i = 0; i < findTriesMax; i++) {
            ...
            if (SandStorm.isDesert(biomeIn, true)) {
                ...
                if (!SandStorm.isDesert(world.getBiomeForCoordsBody(posLeft))) continue;
                ...
                if (!SandStorm.isDesert(world.getBiomeForCoordsBody(posRight))) continue;
                ...
                while (world.isBlockLoaded(posFind) && SandStorm.isDesert(world.getBiomeForCoordsBody(posFind))) {
                    //update last good
    ...
    public void trySpawnStormCloudNearPlayerForLayer(EntityPlayer entP, int layer) {
        ...
        while (tryCountCur++ == 0 || (tryCountCur < tryCountMax && (soClose != null || playerClose != null))) {
            ...
            tryPos = new Vec3(spawnX, CloudStorm.layers.get(layer), spawnZ);
    ...
    public void playerJoinedWorldSyncFull(EntityPlayerMP entP) {
        ...
        World world = getWorld();
        if (world != null) {
            Weather.dbg("Weather2: playerJoinedWorldSyncFull, sending " + getStorms().size() + " weather objects to: " + entP.getName() + ", dim: " + dim);
            //sync storms
            for (int i = 0; i < getStorms().size(); i++) {
    ...
    public void syncStormNew(Storm parStorm, EntityPlayerMP entP) {
        NBTTagCompound data = new NBTTagCompound();
        data.setString("packetCommand", "WeatherData");
    ...
    public void syncStormUpdate(Storm parStorm) {
        ...
        if (testNetworkData) {
            ...
            if (parStorm instanceof CloudStorm) {
                System.out.println("Real: " + ((CloudStorm) parStorm).levelCurIntensityStage);
    ...
    public void syncVolcanoNew(Volcano parStorm, EntityPlayerMP entP) {
        NBTTagCompound data = new NBTTagCompound();
        data.setString("packetCommand", "WeatherData");
    ...
    public void syncVolcanoRemove(Volcano parStorm) {
    ...
    public void syncWeatherVanilla() {
        ...
        data.setBoolean("isVanillaRainActiveOnServer", vanillaRainActiveOnServer);
        data.setBoolean("isVanillaThunderActiveOnServer", vanillaThunderActiveOnServer);
